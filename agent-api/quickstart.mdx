...
Base URL: `https://api.browser.cash`

You can optionally set environment variables in your shell:

- `API_BASE=https://api.browser.cash`
- `API_KEY=<your key>`
...
```bash cURL
curl -H "Authorization: Bearer $API_KEY" \
  "$API_BASE/v1/agent/tasks?pageSize=20&page=1"
```
...
const res = await fetch(`${API_BASE}/v1/agent/tasks?pageSize=20&page=1`, {
  headers: { Authorization: `Bearer ${API_KEY}` },
});
console.log(await res.json());
```
